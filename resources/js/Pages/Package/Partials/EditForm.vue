...
watch(
    ...
    () => {
        if (props.show) {
            form.delivery_company_id = props.item.delivery_company_id;
            form.package_no = props.item.package_no;
            form.recipient_phone_no = props.item.recipient_phone_no;
            form.recipient_unit_id = props.item.recipient_unit_id;
            form.storage_location_id = props.item.storage_location_id;
            let dataDatetime = new Date(props.item.arrived_at);
            dataDatetime = moment(dataDatetime).utc().toString();
            form.arrived_at = dataDatetime;
            form.status = props.item.status;
            form.one_time_storage_fee = props.item.one_time_storage_fee;
...
const editData = () => {
    form.patch(route("packages.update", props.item.id), {
        preserveScroll: true,
        ...
        <Modal :show="show" @close="closeModal" :closeable="false">
            <div class="p-6">
                ...
                <div class="mt-4">
                    ...
                    <InputError
                        :message="form.errors.arrived_at"
                        class="mt-2"
                    />
                </div>
                ...
                <div class="mt-4">
                    ...
                    <UploadFileInput
                        @load:filepond="(filePath) => form.image_path = filePath"
                        target="packages"
                    />
